...
async function runCreditAgent(ctx) {
    return (0, runConfiguredAgent_1.runConfiguredAgent)("CREDIT", ctx, async (currentCtx) => runLocalCreditDecision(currentCtx));
}
function toNumber(value, fallback = 0) {
    const n = Number(value);
    return Number.isFinite(n) ? n : fallback;
}
function estimateEmi(principal, annualRate, months) {
...
function runLocalCreditDecision(currentCtx) {
    const payload = currentCtx.payload || {};
    const applicant = payload.applicant || {};
    const credit = payload.credit || payload.loan || {};
    const monthlyIncome = toNumber(applicant.monthly_income ?? applicant.income_monthly ?? payload.declaredIncome);
    const monthlyLiabilities = toNumber(applicant.monthly_liabilities ?? applicant.emi_outflow ?? 0);
    const cibilScore = toNumber(applicant.cibil_score ?? credit.cibil_score ?? credit.bureau_score);
