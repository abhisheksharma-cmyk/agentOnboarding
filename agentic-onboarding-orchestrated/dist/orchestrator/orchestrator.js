...
function initOrchestrator() {
    ...
    eventBus_1.eventBus.subscribe("onboarding.kyc", async ({ data, traceId }) => {
        ...
        try {
            ...
            const final = (0, decisionGateway_1.evaluateDecision)(agentOutput, ctx);
            ...
            if (final === "APPROVE") {
                ...
                eventBus_1.eventBus.publish("onboarding.address_verification", { ...updatedCtx, stateMachine: nextMachine }, traceId);
            }
            else {
    ...
    eventBus_1.eventBus.subscribe("onboarding.address_verification", async ({ data, traceId }) => {
        ...
        try {
            ...
            const updatedCtx = {
                ...
                payload: {
                    ...ctx.payload,
                    addressResult: agentOutput
                }
            };
            eventBus_1.eventBus.publish("onboarding.address_verification_complete", { agentOutput, final, ctx: updatedCtx, durationMs }, traceId);
            if (final === "APPROVE") {
                const nextMachine = (0, stateMachine_1.transitionState)(updatedMachine, 'START', {});
                (0, stateMachine_1.logStateTransition)(traceId, updatedMachine.currentState, nextMachine.currentState, 'START');
                stateMachines.set(traceId, nextMachine);
                eventBus_1.eventBus.publish("onboarding.aml", { ...updatedCtx, stateMachine: nextMachine }, traceId);
            }
            else {
                eventBus_1.eventBus.publish("onboarding.finished", { final, agentOutput }, traceId);
            }
    ...
    eventBus_1.eventBus.subscribe("onboarding.aml", async ({ data, traceId }) => {
        ...
        try {
            ...
            if (final === "APPROVE") {
                ...
                (0, stateMachine_1.logStateTransition)(traceId, updatedMachine.currentState, nextMachine.currentState, 'START');
                stateMachines.set(traceId, nextMachine);
                eventBus_1.eventBus.publish("onboarding.credit", { ...updatedCtx, stateMachine: nextMachine }, traceId);
            }
            else {
                eventBus_1.eventBus.publish("onboarding.finished", { final, agentOutput }, traceId);
            }
    ...
    eventBus_1.eventBus.subscribe("onboarding.credit", async ({ data, traceId }) => {
        ...
        try {
            ...
            if (final === "APPROVE") {
                ...
                (0, stateMachine_1.logStateTransition)(traceId, updatedMachine.currentState, nextMachine.currentState, 'START');
                stateMachines.set(traceId, nextMachine);
                eventBus_1.eventBus.publish("onboarding.risk", { ...updatedCtx, stateMachine: nextMachine }, traceId);
            }
            else {
...
async function runAndEvaluate(runner, ctx, traceId, stage) {
    ...
    const durationMs = Date.now() - start;
    const final = (0, decisionGateway_1.evaluateDecision)(out, ctx);
