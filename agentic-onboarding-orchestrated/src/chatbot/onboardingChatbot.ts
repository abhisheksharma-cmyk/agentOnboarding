...
export class OnboardingChatbot {
    ...
    async handleMessage(sessionId: string, message: string, files: ProcessedFile[] = []): Promise<string> {
    ...
    private async handleFileUploads(sessionId: string, files: ProcessedFile[]): Promise<void> {
        const userData = this.userSessions.get(sessionId);
        if (!userData) return;
    ...
    private detectFileType(filename: string): string {
        ...
        if (lowerName.includes('aadhaar') || lowerName.includes('aadhar')) return 'aadhaar';
        if (lowerName.includes('pan')) return 'pan';
        if (lowerName.includes('passport')) return 'passport';
        if (lowerName.includes('address') ||
            lowerName.includes('utility') ||
            lowerName.includes('bill')) return 'address';
        return 'other';
    }

    private areAllDocumentsUploaded(userData: UserData): boolean {
    ...
    private async startVerification(sessionId: string): Promise<void> {
        const userData = this.userSessions.get(sessionId);
        if (!userData) return;
        ...
        try {
            ...
            if (userData.address) {
                ...
                if (!userData.verificationStatus) userData.verificationStatus = {};
            ...
            eventBus.publish('onboarding.verification_complete', {
                userId: sessionId,
            ...
            eventBus.publish('onboarding.verification_failed', {
                userId: sessionId,
    ...
    private async verifyAddress(userData: UserData): Promise<any> {
    ...
    private async runKycVerification(userData: UserData): Promise<any> {
        const kycContext = {
        ...
        return await runKycAgent(kycContext);
    }

    private isVerificationComplete(userData: UserData): boolean {
    ...
    private async fileToBase64(file: ProcessedFile): Promise<string> {
        // Convert buffer to base64
        return `data:${file.mimetype};base64,${file.buffer.toString('base64')}`;
    }

    // Public method to get current session state
    getSessionState(sessionId: string): string | undefined {
        return this.sessionStates.get(sessionId);
    }

    // Public method to get user data (for debugging or admin purposes)
    getUserData(sessionId: string): UserData | undefined {
        return this.userSessions.get(sessionId);
    }
}

// Singleton instance
export const onboardingChatbot = new OnboardingChatbot();
