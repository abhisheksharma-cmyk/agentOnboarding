...
export function evaluateDecision(out: AgentOutput, ctx?: AgentContext): FinalDecision {
  const riskProfile = normalizeRisk((
    ctx?.payload?.riskProfile ??
...
function normalizeRisk(val: string): "high" | "low" | "" {
...
function documentMatchesApplication(ctx: AgentContext, out: AgentOutput, risk: "high" | "low" | ""): boolean {
  ...
  // Also check extractedFields as a fallback
  const extractedFields = ctx?.payload?.extractedFields ?? {};

  console.log("[DecisionGateway] DEBUG - Full context payload:", JSON.stringify(ctx?.payload, null, 2));
  console.log("[DecisionGateway] DEBUG - Documents array:", documents);
  console.log("[DecisionGateway] DEBUG - Document object:", doc);
  console.log("[DecisionGateway] DEBUG - Extracted fields:", extractedFields);

  const appName = normalizeString(
    applicant.fullName ?? applicant.name ?? applicant.full_name
  );
  ...
  // Try multiple sources for document data
  const docName = normalizeString(
    doc.fullName ?? doc.name ?? extractedFields.fullName ?? ctx?.payload?.extractedFields?.fullName
  );
  const docGender = normalizeString(
    doc.gender ?? extractedFields.gender ?? ctx?.payload?.extractedFields?.gender
  );
  const docDob = normalizeDate(
    doc.dateOfBirth ?? doc.dob ?? extractedFields.dateOfBirth ?? ctx?.payload?.extractedFields?.dateOfBirth
  );

  // Document address: try verified_address from agent metadata or doc fields
  const verifiedAddress =
    normalizeString(out?.metadata?.verified_address ?? out?.verified_address);
  const docAddressObj = doc.address ?? extractedFields.address ?? {};
...
function namesMatchWithInitials(docName: string, applicantName: string): boolean {
  const dt = tokenizeName(docName);
  const at = tokenizeName(applicantName);
  if (!dt.length || !at.length) return false;
  if (dt.length !== at.length) return false;
  for (let i = 0; i < dt.length; i++) {
    if (dt[i] === at[i]) continue;
    if (isInitialToken(dt[i]) && at[i].startsWith(dt[i])) continue;
    return false;
  }
  return true;
}
